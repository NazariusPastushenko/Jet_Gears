...
namespace Jet_Gears.Controls
{
    public class button : Button
    {
        ...
        [Description(
            "Указывает, включено ли использование отдельного цвета обводки (границы) кнопки при наведении курсора")]
        public bool BorderColorOnHoverEnabled { get; set; } = false;
        

        [Description(
            "Дополнительный фоновый цвет кнопки используемый для создания градиента (При BackColorGradientEnabled = true)")]
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            Rectangle rectTextHover =
                new Rectangle((int)TextSlideAnim.Value - rect.Width, rect.Y, rect.Width, rect.Height);
            ...
            else
            {
                ...
                for (int i = 0; i < RippleButtonAnimDic.Count; i++)
                {
                    ...
                    if (MultiRippleButtonAnim.Value > 0 &&
                        MultiRippleButtonAnim.Value < MultiRippleButtonAnim.TargetValue)
                    {
                ...
                List<Animation> completedRippleAnimations = RippleButtonAnimDic.Keys.ToList()
                    .FindAll(x => x.Status == Animation.AnimationStatus.Completed);
        ...
        private void ButtonCurtainAction()
        {
            if (MouseEntered)
            {
                CurtainButtonAnim = new Animation("ButtonCurtain_" + Handle, Invalidate, CurtainButtonAnim.Value,
                    Width - 1);
            }
            else
            {
